...
function renderFriendModal(id) {
  ...
  const addFavoriteBtn = document.querySelector('#friend-modal-add-favorite')
  ...
  axios.get(`${INDEX_URL}/${id}`)
    ...
    `
      addFavoriteBtn.dataset.id = data.id
...
function renderPaginator(currentPage) {
  ...
  pageArray.forEach(page => {
    if (page > 0 && page <= TOTAL_PAGES) {
      if (page === currentPage) {
        rawHTML += `
        <li class="page-item active"><a class="page-link page-number" href="#data-panel" data-page="${page}">${page}</a></li>
      `
      } else {
        rawHTML += `
        <li class="page-item"><a class="page-link page-number" href="#data-panel" data-page="${page}">${page}</a></li>
      `
      }
    }
  })
...
modalFooter.addEventListener('click', function onModalFooterClicked(event) {
  if (event.target.matches('#friend-modal-add-favorite')) {
    addToFavorites(Number(event.target.dataset.id))
  }
})

// Initialize the frind list and invoke the function to render
axios.get(INDEX_URL)
  .then(rep => {
    friendList.push(...rep.data.results)
    renderFriendList(getFriendDataByPage(1))
